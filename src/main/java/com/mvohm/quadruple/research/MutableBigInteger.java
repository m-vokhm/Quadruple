...
import static com.mvohm.quadruple.research.BigDecimal.INFLATED;
...
public class MutableBigInteger {
    ...
    private long toLong() {
        ...
        if (intLen == 0)
            return 0;
        final long d = value[offset] & LONG_MASK;
    ...
    final int compareHalf(MutableBigInteger b) {
        ...
        for (int i = offset, j = bstart; i < len + offset;) {
            final int bv = bval[j++];
            final long hb = ((bv >>> 1) + carry) & LONG_MASK;
            final long v = val[i++] & LONG_MASK;
            if (v != hb)
                return v < hb ? -1 : 1;
    ...
    void multiply(MutableBigInteger y, MutableBigInteger z) {
        ...
        for (int j=yLen-1, k=yLen+xLen-1; j >= 0; j--, k--) {
                final long product = (y.value[j+y.offset] & LONG_MASK) *
    ...
    void mul(int y, MutableBigInteger z) {
        ...
        for (int i = intLen-1; i >= 0; i--) {
            final long product = ylong * (value[i+offset] & LONG_MASK) + carry;
    ...
    int divideOneWord(int divisor, MutableBigInteger quotient) {
        ...
        if (intLen == 1) {
            final long dividendValue = value[offset] & LONG_MASK;
...
//      say(">>> exiting divideMagnitude");
    ...
    private MutableBigInteger divideLongMagnitude(long ldivisor, MutableBigInteger quotient) {
      ...
      final long dhLong = dh & LONG_MASK;
      final int dl = (int)(ldivisor & LONG_MASK);
    ...
    MutableBigInteger mutableModInverse(MutableBigInteger p) {
        ...
        final int powersOf2 = p.getLowestSetBit();

        // Construct odd part of modulus
        final MutableBigInteger oddMod = new MutableBigInteger(p);
        ...
        final MutableBigInteger oddPart = modInverse(oddMod);

        // Calculate 1/a mod evenMod
        final MutableBigInteger evenPart = modInverseMP2(powersOf2);

        // Combine the results using Chinese Remainder Theorem
        final MutableBigInteger y1 = modInverseBP2(oddMod, powersOf2);
        final MutableBigInteger y2 = oddMod.modInverseMP2(powersOf2);

        final MutableBigInteger temp1 = new MutableBigInteger();
        final MutableBigInteger temp2 = new MutableBigInteger();
        final MutableBigInteger result = new MutableBigInteger();
    ...
    private MutableBigInteger modInverse(MutableBigInteger mod) {
        final MutableBigInteger p = new MutableBigInteger(mod);
        ...
        if (f.isEven()) {
            final int trailingZeros = f.getLowestSetBit();
