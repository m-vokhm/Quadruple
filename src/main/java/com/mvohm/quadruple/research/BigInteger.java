...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    BigInteger(char[] val, int sign, int len) {
        ...
        if (len < 10) {
            ...
            final long numBits = ((numDigits * bitsPerDigit[10]) >>> 10) + 1;
            if (numBits + 31 >= (1L << 32)) {
                reportOverflow();
            }
        ...
        // Process remaining digit groups
        while (cursor < len) {
            final int groupVal = parseInt(val, cursor, cursor += digitsPerInt[10]);
    ...
    private int parseInt(char[] source, int start, int end) {
        ...
        for (int index = start; index < end; index++) {
            final int nextVal = Character.digit(source[index], 10);
            if (nextVal == -1)
                throw new NumberFormatException(new String(source));
    ...
    private static void destructiveMulAdd(int[] x, int y, int z) {
        ...
        final long ylong = y & LONG_MASK;
        final long zlong = z & LONG_MASK;
    ...
    private static byte[] randomBits(int numBits, Random rnd) {
        ...
        final int numBytes = (int)(((long)numBits+7)/8); // avoid overflow
        final byte[] randomBits = new byte[numBytes];
    ...
    private static BigInteger smallPrime(int bitLength, int certainty, Random rnd) {
        final int magLen = (bitLength + 31) >>> 5;
        final int temp[] = new int[magLen];
        final int highBit = 1 << ((bitLength+31) & 0x1f);  // High bit of high int
        final int highMask = (highBit << 1) - 1;  // Bits to keep in high int
        ...
        while (true) {
            ...
            final BigInteger p = new BigInteger(temp, 1);

            // Do cheap "pre-test" if applicable
            if (bitLength > 6) {
                final long r = p.remainder(SMALL_PRIME_PRODUCT).longValue();
    ...
    private static BigInteger largePrime(int bitLength, int certainty, Random rnd) {
        ...
        final int searchLen = getPrimeSearchLen(bitLength);
    ...
    */
    public BigInteger nextProbablePrime() {
        ...
        if (result.bitLength() < SMALL_PRIME_THRESHOLD) {
            ...
            while (true) {
                ...
                if (result.bitLength() > 6) {
                    final long r = result.remainder(SMALL_PRIME_PRODUCT).longValue();
                    if ((r%3==0)  || (r%5==0)  || (r%7==0)  || (r%11==0) ||
                        (r%13==0) || (r%17==0) || (r%19==0) || (r%23==0) ||
                        (r%29==0) || (r%31==0) || (r%37==0) || (r%41==0)) {
        ...
        final int searchLen = getPrimeSearchLen(result.bitLength());

        while (true) {
           final BitSieve searchSieve = new BitSieve(result, searchLen);
           final BigInteger candidate = searchSieve.retrieve(result,
    ...
    boolean primeToCertainty(int certainty, Random random) {
        int rounds = 0;
        final int n = (Math.min(certainty, Integer.MAX_VALUE-1)+1)/2;

        // The relationship between the certainty and the number of rounds
        // we perform is given in the draft standard ANSI X9.80, "PRIME
        // NUMBER GENERATION, PRIMALITY TESTING, AND PRIMALITY CERTIFICATES".
        final int sizeInBits = this.bitLength();
        if (sizeInBits < 100) {
            rounds = 50;
    ...
    private boolean passesLucasLehmer() {
        final BigInteger thisPlusOne = this.add(ONE);
        ...
        // Step 2
        final BigInteger u = lucasLehmerSequence(d, thisPlusOne, this);
    ...
    private static BigInteger lucasLehmerSequence(int z, BigInteger k, BigInteger n) {
        final BigInteger d = BigInteger.valueOf(z);
    ...
    private boolean passesMillerRabin(int iterations, Random rnd) {
        ...
        final BigInteger thisMinusOne = this.subtract(ONE);
        BigInteger m = thisMinusOne;
        final int a = m.getLowestSetBit();
        m = m.shiftRight(a);
    ...
    private static int[] subtract(long val, int[] little) {
        ...
        if (highWord == 0) {
            ...
            if (little.length == 1) {
                final long difference = ((int)val & LONG_MASK) - (little[0] & LONG_MASK);
    ...
    private static BigInteger multiplyByInt(int[] x, int y, int sign) {
        ...
        long carry = 0;
        final long yl = y & LONG_MASK;
        int rstart = rmag.length - 1;
        for (int i = xlen - 1; i >= 0; i--) {
            final long product = (x[i] & LONG_MASK) * yl + carry;
    ...
    BigInteger multiply(long v) {
        ...
        int rstart = rmag.length - 1;
        for (int i = xlen - 1; i >= 0; i--) {
            final long product = (value[i] & LONG_MASK) * dl + carry;
            rmag[rstart--] = (int)product;
            carry = product >>> 32;
        }
        rmag[rstart] = (int)carry;
        if (dh != 0L) {
            ...
            for (int i = xlen - 1; i >= 0; i--) {
                final long product = (value[i] & LONG_MASK) * dh +
    ...
    private static int[] multiplyToLen(int[] x, int xlen, int[] y, int ylen, int[] z) {
        ...
        for (int j=ystart, k=ystart+1+xstart; j >= 0; j--, k--) {
            final long product = (y[j] & LONG_MASK) *
    ...
    private static BigInteger multiplyKaratsuba(BigInteger x, BigInteger y) {
        ...
        final BigInteger result = p1.shiftLeft(32*half).add(p3.subtract(p1).subtract(p2)).shiftLeft(32*half).add(p2);
    ...
    private static BigInteger multiplyToomCook3(BigInteger a, BigInteger b) {
        ...
        final BigInteger result = vinf.shiftLeft(ss).add(t2).shiftLeft(ss).add(t1).shiftLeft(ss).add(tm1).shiftLeft(ss).add(v0);
    ...
    private static final int[] squareToLen(int[] x, int len, int[] z) {
     ...
     private static final int[] implSquareToLen(int[] x, int len, int[] z, int zlen) {
        ...
        for (int j=0, i=0; j < len; j++) {
            final long piece = (x[j] & LONG_MASK);
    ...
    private BigInteger[] divideAndRemainderBurnikelZiegler(BigInteger val) {
        final MutableBigInteger q = new MutableBigInteger();
        final MutableBigInteger r = new MutableBigInteger(this).divideAndRemainderBurnikelZiegler(new MutableBigInteger(val), q);
        final BigInteger qBigInt = q.isZero() ? ZERO : q.toBigInteger(signum*val.signum);
        final BigInteger rBigInt = r.isZero() ? ZERO : r.toBigInteger(signum);
    ...
    public BigInteger pow(int exponent) {
        ...
        if (partToSquare.mag.length == 1 && scaleFactor <= 62) {
            ...
            final int newSign = (signum <0  && (exponent&1) == 1 ? -1 : 1);
            long result = 1;
    ...
    private static int[] leftShift(int[] a, int len, int n) {
        ...
        final int bitsInHighWord = bitLengthForInt(a[0]);
        ...
        if (n <= (32-bitsInHighWord)) {
            ...
            if (nBits <= (32-bitsInHighWord)) {
                final int result[] = new int[nInts+len];
                System.arraycopy(a, 0, result, 0, len);
                primitiveLeftShift(result, result.length, nBits);
                return result;
            } else {
                final int result[] = new int[nInts+len+1];
                System.arraycopy(a, 0, result, 0, len);
    ...
    public BigInteger modPow(BigInteger exponent, BigInteger m) {
        ...
        final BigInteger base = (this.signum < 0 || this.compareTo(m) >= 0
                           ? this.mod(m) : this);
        BigInteger result;
        if (m.testBit(0)) { // odd modulus
            ...
            final int p = m.getLowestSetBit();   // Max pow of 2 that divides m

            final BigInteger m1 = m.shiftRight(p);  // m/2**p
            final BigInteger m2 = ONE.shiftLeft(p); // 2**p

            // Calculate new base from m1
            final BigInteger base2 = (this.signum < 0 || this.compareTo(m1) >= 0
                                ? this.mod(m1) : this);

            // Caculate (base ** exponent) mod m1.
            final BigInteger a1 = (m1.equals(ONE) ? ZERO :
                             base2.oddModPow(exponent, m1));

            // Calculate (this ** exponent) mod m2
            final BigInteger a2 = base.modPow2(exponent, p);

            // Combine results using Chinese Remainder Theorem
            final BigInteger y1 = m2.modInverse(m1);
            final BigInteger y2 = m1.modInverse(m2);

            if (m.mag.length < MAX_MAG_LENGTH / 2) {
                result = a1.multiply(m2).multiply(y1).add(a2.multiply(m1).multiply(y2)).mod(m);
            } else {
                final MutableBigInteger t1 = new MutableBigInteger();
                new MutableBigInteger(a1.multiply(m2)).multiply(new MutableBigInteger(y1), t1);
                final MutableBigInteger t2 = new MutableBigInteger();
     ...
     */
        ...
        final int tblmask = 1 << wbits;

        // Allocate table for precomputed odd powers of base in Montgomery form
        final int[][] table = new int[tblmask][];
        for (int i=0; i < tblmask; i++)
            table[i] = new int[modLen];

        // Compute the modular inverse of the least significant 64-bit
        // digit of the modulus
        final long n0 = (mod[modLen-1] & LONG_MASK) + ((mod[modLen-2] & LONG_MASK) << 32);
        final long inv = -MutableBigInteger.inverseMod64(n0);
        ...
        final MutableBigInteger r= a2.divide(b2, q);
        table[0] = r.toIntArray();

        // Pad table[0] with leading zeros so its length is at least modLen
        if (table[0].length < modLen) {
           final int offset = modLen - table[0].length;
           final int[] t2 = new int[modLen];
    ...
    private static int[] montReduce(int[] n, int[] mod, int mlen, int inv) {
        ...
        do {
            final int nEnd = n[n.length-1-offset];
            final int carry = mulAdd(n, mod, offset, mlen, inv * nEnd);
    ...
    private static int intArrayCmpToLen(int[] arg1, int[] arg2, int len) {
        for (int i=0; i < len; i++) {
            final long b1 = arg1[i] & LONG_MASK;
            final long b2 = arg2[i] & LONG_MASK;
            if (b1 < b2)
                return -1;
            if (b1 > b2)
                return 1;
    ...
    private static int implMulAdd(int[] out, int[] in, int offset, int len, int k) {
        final long kLong = k & LONG_MASK;
        long carry = 0;

        offset = out.length-offset - 1;
        for (int j=len-1; j >= 0; j--) {
            final long product = (in[j] & LONG_MASK) * kLong +
    ...
    static int addOne(int[] a, int offset, int mlen, int carry) {
        ...
        final long t = (a[offset] & LONG_MASK) + (carry & LONG_MASK);
    ...
    private BigInteger mod2(int p) {
        ...
        final int numInts = (p + 31) >>> 5;
        final int[] mag = new int[numInts];
        System.arraycopy(this.mag, (this.mag.length - numInts), mag, 0, numInts);

        // Mask out any excess bits
        final int excessBits = (numInts << 5) - p;
    ...
    public BigInteger modInverse(BigInteger m) {
        ...
        final MutableBigInteger a = new MutableBigInteger(modVal);
        final MutableBigInteger b = new MutableBigInteger(m);

        final MutableBigInteger result = a.mutableModInverse(b);
        return result.toBigInteger(1);
    }
    ...
    public BigInteger and(BigInteger val) {
        final int[] result = new int[Math.max(intLength(), val.intLength())];
    ...
    public BigInteger clearBit(int n) {
        ...
        final int intNum = n >>> 5;
        final int[] result = new int[Math.max(intLength(), ((n + 1) >>> 5) + 1)];
    ...
    public int bitLength() {
        ...
        if (n == -1) { // bitLength not initialized yet
            ...
            if (len == 0) {
                ...
                final int magBitLength = ((len - 1) << 5) + bitLengthForInt(mag[0]);
                 if (signum < 0) {
    ...
    private String smallToString(int radix) {
        ...
        final StringBuilder buf = new StringBuilder(numGroups*digitsPerLong[radix]+1);
        if (signum < 0) {
            buf.append('-');
        }
        ...
        for (int i=numGroups-2; i >= 0; i--) {
            ...
            final int numLeadingZeros = digitsPerLong[radix]-digitGroup[i].length();
    ...
    public float floatValue() {
        ...
        final int exponent = ((mag.length - 1) << 5) + bitLengthForInt(mag[0]) - 1;
         ...
         */
        final int shift = exponent - FloatConsts.SIGNIFICAND_WIDTH;
         ...
         */
        final boolean increment = (twiceSignifFloor & 1) != 0
    ...
    public double doubleValue() {
         ...
         * 2^(SIGNIFICAND_WIDTH - 1 - exponent).
         */
        final int shift = exponent - DoubleConsts.SIGNIFICAND_WIDTH;
    ...
    private static int[] stripLeadingZeroBytes(byte a[]) {
        ...
        final int intLength = ((byteLength - keep) + 3) >>> 2;
        final int[] result = new int[intLength];
        int b = byteLength - 1;
    ...
    private static int[] makePositive(byte a[]) {
        ...
        final int extraByte = (k == byteLength) ? 1 : 0;
        final int intLength = ((byteLength - keep + extraByte) + 3) >>> 2;
        final int result[] = new int[intLength];
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        final int sign = fields.get("signum", -2);
        final byte[] magnitude = (byte[])fields.get("magnitude", null);
        ...
        final int[] mag = stripLeadingZeroBytes(magnitude);
    ...
    private byte[] magSerializedForm() {
        ...
        final int bitLen = (len == 0 ? 0 : ((len - 1) << 5) + bitLengthForInt(mag[0]));
        final int byteLen = (bitLen + 7) >>> 3;
